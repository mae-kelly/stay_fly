...
class TokenSecurityAnalyzer:
    def __init__(self):
        ...
        self.dangerous_functions = {
            ...
            # Fee modification
            'fee_modify': ['setfee', 'settax', 'updatefee', 'changefee', 'setbuyfee', 'setsellfee'],
            # Ownership
    ...
    async def analyze_token(self, token_address: str) -> SecurityAnalysis:
        ...
        contract_verified = results[0] if not isinstance(results[0], Exception) else False
                ...
                if response.status == 200:
                    ...
                    if pairs:
                        ...
                        liquidity_info.update({
                            'liquidity_eth': float(best_pair.get('liquidity', {}).get('base', 0)),
                            'liquidity_usd': float(best_pair.get('liquidity', {}).get('usd', 0)),
                            'pool_address': best_pair.get('pairAddress')
                        })
            
            # Check if liquidity is locked (simplified check)
            if liquidity_info['liquidity_usd'] > 50000:  # $50K+ usually indicates some stability
            ...
            async with self.session.get(url, params=params) as response:
                ...
                if data.get('status') == '1' and data.get('result'):
                    ...
                    if source_code:
                        ...
                        if self.web3:
                            bytecode = self.web3.eth.get_code(Web3.toChecksumAddress(token_address))
                            bytecode_analysis.update(self.analyze_bytecode_patterns(bytecode.hex()))
    ...
    def analyze_source_code(self, source_code: str) -> Dict:
        ...
        blacklist_patterns = ['blacklist', 'isblacklisted', '_blacklist', 'addtoblacklist']
        analysis['blacklist'] = any(pattern in source_code for pattern in blacklist_patterns)
        ...
        analysis['max_tx_limit'] = any(pattern in source_code for pattern in limit_patterns)
        
        # Check for fee modification
        fee_patterns = ['setfee', 'settax', 'updatefee', 'changefee']
        analysis['fee_modifiable'] = any(pattern in source_code for pattern in fee_patterns)
        
        # Check for trading control
        trading_patterns = ['enabletrading', 'disabletrading', 'tradingactive']
        analysis['trading_controllable'] = any(pattern in source_code for pattern in trading_patterns)
                ...
                if response.status == 200:
                    ...
                    if simulation:
                        ...
                        honeypot_info.update({
                            'buy_tax': buy_tax,
                            'sell_tax': sell_tax,
                            'high_tax': (buy_tax + sell_tax) > 10.0,  # > 10% total tax
                            'transfer_pausable': not simulation.get('transferSuccessful', True)
                        })
                        
                        # Calculate risk score
                        total_tax = buy_tax + sell_tax
                        honeypot_info['honeypot_risk'] = min(total_tax / 20.0, 1.0)  # Max 20% = 100% risk
                    ...
                    if data.get('status') == '1' and data.get('result'):
                        ...
                        if holders:
                            ...
                            if total_supply > 0:
                                ...
                                holder_stats['top_holder_percentage'] = (top_holder_balance / total_supply) * 100
                                
                                # Flag concentration risk if top holder has >50%
                                holder_stats['concentration_risk'] = holder_stats['top_holder_percentage'] > 50.0
    ...
    async def is_safe_to_trade(self, token_address: str, min_score: float = 70.0) -> Tuple[bool, SecurityAnalysis]:
        ...
        is_safe = analysis.safety_score >= min_score and analysis.risk_level in ['LOW', 'MEDIUM']
...
async def main():
    ...
    async with analyzer:
        ...
        for token_addr in demo_tokens:
            ...
            try:
                safe, analysis = await analyzer.is_safe_to_trade(token_addr, min_score=60.0)
                
                print(f"📊 Results:")
                print(f"   Contract Verified: {'✅' if analysis.contract_verified else '❌'}")
                print(f"   Ownership Renounced: {'✅' if analysis.ownership_renounced else '❌'}")

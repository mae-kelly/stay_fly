...
class OKXLiveEngine:
    ...
    def create_okx_signature(self, timestamp: str, method: str, request_path: str, body: str = "") -> str:
    ...
    def get_okx_headers(self, method: str, request_path: str, body: str = "") -> Dict[str, str]:
    ...
    async def get_dex_quote(self, from_token: str, to_token: str, amount: str) -> Optional[OKXQuote]:
    ...
    def simulate_dex_quote(self, from_token: str, to_token: str, amount: str) -> OKXQuote:
        """Simulate DEX quote for testing"""
        # Simulate reasonable quote
        from_amount_float = float(amount) / 1e18  # Convert from wei
        simulated_output = from_amount_float * 1000 * 1e18  # Simulate 1 ETH = 1000 tokens
    ...
    async def execute_live_trade(
        ...
        logger.info(f"🚀 EXECUTING {'SIMULATED' if self.simulation_mode else 'LIVE'} TRADE")
        ...
        if swap_result['success']:
            logger.info(f"✅ {'SIMULATED' if self.simulation_mode else 'LIVE'} TRADE SUCCESSFUL ({execution_time_ms:.1f}ms)")
            ...
            logger.error(f"❌ {'SIMULATED' if self.simulation_mode else 'LIVE'} TRADE FAILED ({execution_time_ms:.1f}ms)")
    ...
    async def update_positions(self):
        ...
        for token_address, position in self.positions.items():
            try:
                ...
                logger.info(f"📊 {position.token_symbol}: ${current_value:.2f} ({multiplier:.2f}x) | P&L: ${unrealized_pnl:+.2f}")
    ...
    async def close_position(self, token_address: str, reason: str):
        ...
        if self.simulation_mode:
            ...
            sell_quote = await self.get_dex_quote(token_address, weth_address, token_amount)
            if sell_quote:
                sell_result = await self.execute_okx_swap(sell_quote, 0)
                if sell_result['success']:
                    logger.info(f"✅ Sell order executed: {sell_result.get('tx_hash', '')[:10]}...")
    ...
    async def get_portfolio_summary(self) -> Dict:
        ...
        return {
            ...
            'total_return_pct': ((total_current_value + realized_pnl) / 1000.0 - 1.0) * 100 if total_current_value + realized_pnl > 0 else 0,
    ...
    async def emergency_close_all(self):
        ...
        logger.info(f"✅ Emergency close complete: {len(position_addresses)} positions closed")
    ...
    def save_session(self, filename: str = None):
        ...
        for pos_data in session_data['positions'].values():
            if 'entry_time' in pos_data and isinstance(pos_data['entry_time'], datetime):

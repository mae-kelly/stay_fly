...
class OKXDEXClient:
    ...
    async def get_quote(
        self, from_token: str, to_token: str, amount: str, chain_id: str = "1"
    ) -> Dict[str, Any]:
        params = {
            "chainId": chain_id,
        ...
        async with self.session.get(
            url,
            params=params,
            headers=self._get_headers("GET", "/api/v5/dex/aggregator/quote"),
        ) as response:
            return await response.json()

    async def execute_swap(
        self, quote_data: Dict[str, Any], user_wallet: str
    ) -> Dict[str, Any]:
        swap_data = {
        ...
        async with self.session.post(
            url,
            data=body,
            headers=self._get_headers("POST", "/api/v5/dex/aggregator/swap", body),
        ) as response:
            return await response.json()
    ...
    async def get_token_info(
        ...
        async with self.session.get(
            url,
            params=params,
            headers=self._get_headers("GET", "/api/v5/dex/aggregator/tokens"),
        ) as response:
            return await response.json()
    ...
    async def mirror_trade_execution(
        self, token_address: str, amount_eth: float, wallet_address: str
    ) -> Dict[str, Any]:
        try:
            quote = await self.get_quote(
                "0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE",
            ...
            simulation = await self.simulate_trade(
                TradeParams(symbol=token_address, side="buy", amount=amount_eth)
            )
